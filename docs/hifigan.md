...
当前性能/精度数字与部署定位统一见 `benchmarks.md`(§1 当前数据、§6 终态裁决)。历史快照
(patch 落地前)见 `verification_real_hifigan.md` §3——仅存档,不要按那里的数来评估现状。

- **主线(fp32,speed 基线外置)**:见 `benchmarks.md` §1;实用数量级:RTX 2070 上 Vulkan 433–480 ms
  处理一条 20 s 音频,CPU 16 线程 ≈4.5 s(≈4.4× 实时);最低延迟路径仍为 ONNX/DML(刻意止步,
  见 `benchmarks.md` §6)。
- **实现要点对性能的直接后果**:每帧 512 采样输出成本由卷积决定;`hifigan_run` 逐条重建 graph
  (server 无状态 API);sub-pixel 在 CPU/Vulkan 走 F32 im2col+mul_mat。
